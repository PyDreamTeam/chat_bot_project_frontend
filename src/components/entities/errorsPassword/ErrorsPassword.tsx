          ...
          <div>
               {errPas.map((item) => (
                    <div key={item.condition} className={css.errors}>
                         {error.includes(item.condition) ? <Image src={item.iconNotDone} alt="icon" width={24} height={24}/> : <Image src={item.iconDone} alt="icon" width={24} height={24}/>}
                         <Text type="reg14" color={error.includes(item.condition) ? "red" : "green"}>{item.condition}</Text>
                    </div>
               ))}
          </div>
     );
