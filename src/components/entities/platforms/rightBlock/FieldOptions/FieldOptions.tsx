               ...
               <ul className={css.list}>
                    {filters.map((item) => (
                         <li key={item.id} className={css.option}>
                              <Text type="reg14" color="telegray">{item.filter}{item.filter !== "" && ":"} </Text>
                              <Text type="reg14" color="dark">{item.tag}</Text>
                              <Image src={"/img/close.svg"} alt="close" width={24} height={24} onClick={() => dispatch(deleteFilters(item.id))} className={css.img}/>
                         </li>
                    ))}
                    {filters.length !== 0 && <button onClick={() => dispatch(deleteAllFilters())} className={css.btn}>Очистить все</button>}
               </ul>
          </div>
     );
};